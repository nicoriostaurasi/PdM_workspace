...
int main(void)
{
  ...
  delay_t myDelay;
  int myBlinkTimesController = 0;
  int myPeriodController = 0;
  ...
  while(1)
  {
	  for(myPeriodController=0;myPeriodController<3;myPeriodController++)
	  {
		delayWrite(&myDelay,delayVector[myPeriodController]);
		myBlinkTimesController=0;
		do{
			if(delayRead(&myDelay)){
				HAL_GPIO_TogglePin(LD2_GPIO_Port, LD2_Pin);
				myBlinkTimesController++;
			}
		}
		while(myBlinkTimesController<TOGGLE_TIMES);
	  }
  }
}
